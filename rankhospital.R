rankhospital <- function(state, outcome, num = "best") {
    source("C:\\Users\\matheus\\Documents\\GitHub\\ProgrammingAssignment3\\best.R")
    ...
    if(num == "best"){
        ...
        print( evaluate_data[ order(evaluate_data[,2], evaluate_data[,1]), ][dim(evaluate_data)[1],1] )
    }
    else {
        num <- as.numeric(num) 
        print( evaluate_data[ order(evaluate_data[,2], evaluate_data[,1]), ][num,1])
    }
}
